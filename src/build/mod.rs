...
pub(crate) fn build(
    ...
    let image_exists = existing_image_created.is_some();
    ...
    let needs_rebuild = config.force_rebuild
        || should_rebuild(config, existing_image_created, filesystem, clock)
            .map_err(BuildError::StalenessCheck)?;
    if !needs_rebuild {
...
///
/// Returns `true` if:
/// - The image does not exist (`existing_image_created` is `None`).
/// - The Dockerfile was modified after the image was created.
/// - The image was created before the start of today (local time).
fn should_rebuild(
    config: &Config,
    existing_image_created: Option<DateTime<Utc>>,
    filesystem: &impl Filesystem,
    clock: &impl Clock,
) -> Result<bool, anyhow::Error> {
    let Some(image_created) = existing_image_created else {
        return Ok(true);
    };
    ...
    if dockerfile_mtime > image_created {
        return Ok(true);
    }
    ...
    let image_created_local = image_created.with_timezone(now_local.offset()).date_naive();
    if image_created_local < today_local {
        return Ok(true);
    }
